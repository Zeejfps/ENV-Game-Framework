...
layout (location = 4) out vec3 out_metallic_roughness;
...
void main()
{
    vec3 color = texture(material.albedo, fs_in.tex_coord).rgb;
    vec3 roughness = texture(material.roughness, fs_in.tex_coord).rgb;
    vec3 metallic = texture(material.metallic, fs_in.tex_coord).rgb;
    ...
    out_metallic_roughness = vec3(metallic.r, roughness.r, occlusion.r);
