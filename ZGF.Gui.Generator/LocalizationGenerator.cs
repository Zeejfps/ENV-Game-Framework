...
namespace ZGF.Gui.Generator;

/// <summary>
/// Emits a runtime-switchable localization catalog for any app that uses ZGF.Gui. The app supplies
/// <c>Localization/Strings/*.json</c> catalogs (as <c>AdditionalFiles</c>) and a hand-authored
/// <c>Locale</c> enum; the generator bakes a <c>Strings</c> class plus the per-app facade
/// (<c>ILocalizationService</c>, <c>LocalizationService</c>, <c>L</c>, <c>ctx.Localization()</c>)
/// into the namespace the <c>Locale</c> enum lives in, on top of <c>ZGF.Gui.Localization</c>.
/// </summary>
[Generator]
public sealed class LocalizationGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor NoLocaleEnum = new(
        id: "LOC007",
        title: "No Locale enum found",
        messageFormat: "Found localization catalogs but no 'Locale' enum; declare one so the generated catalog has a home namespace",
        category: "Localization",
        DiagnosticSeverity.Error,
        isEnabledByDefault: true);
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var localeCases = context.SyntaxProvider
            .CreateSyntaxProvider(
                ...
                static (ctx, _) =>
                {
                    var e = (EnumDeclarationSyntax)ctx.Node;
                    return (
                        Namespace: NamespaceOf(e),
                        Members: e.Members.Select(static m => m.Identifier.ValueText).ToImmutableArray());
                })
            .Collect();
    ...
    private static void Generate(
        ...
        ImmutableArray<(string? Namespace, ImmutableArray<string> Members)> declaredCases)
    {
        ...
        var targetNamespace = declaredCases
            .Select(static c => c.Namespace)
            .FirstOrDefault(static n => !string.IsNullOrEmpty(n));
        if (string.IsNullOrEmpty(targetNamespace))
        {
            spc.ReportDiagnostic(Diagnostic.Create(NoLocaleEnum, Location.None));
            return;
        }

        var declaredMembers = new HashSet<string>(declaredCases.SelectMany(static c => c.Members));
        var hasPseudo = declaredMembers.Contains("Pseudo");
        ...
        spc.AddSource("Strings.g.cs",
            SourceText.From(Emit(targetNamespace!, hasPseudo, locales, referenceByKey, referenceKeys), Encoding.UTF8));
    }
        ...
        List<string> referenceKeys)
    {
        ...
        sb.AppendLine("using ZGF.Gui;");
        sb.AppendLine("using ZGF.Gui.Localization;");
        sb.AppendLine("using ZGF.Observable;");
        ...
        EmitFacade(sb, targetNamespace);
        return sb.ToString();
    }

    private static void EmitFacade(StringBuilder sb, string targetNamespace)
    {
        sb.AppendLine();
        sb.AppendLine("/// <summary>App-local catalog contract. Inject or resolve this; it carries the active");
        sb.AppendLine("/// <c>Strings</c> as an observable so widgets re-render on a locale switch.</summary>");
        sb.AppendLine("public interface ILocalizationService : global::ZGF.Gui.Localization.ILocalizationService<Strings> { }");
        sb.AppendLine();
        sb.AppendLine("internal sealed class LocalizationService : ILocalizationService, global::System.IDisposable");
        sb.AppendLine("{");
        sb.AppendLine("    private readonly global::ZGF.Observable.Derived<Strings> _strings;");
        sb.AppendLine("    public global::ZGF.Observable.IReadable<Strings> Strings => _strings;");
        sb.AppendLine("    public LocalizationService(global::ZGF.Observable.State<Locale> locale)");
        sb.AppendLine("    {");
        sb.AppendLine($"        _strings = new global::ZGF.Observable.Derived<Strings>(() => global::{targetNamespace}.Strings.For(locale.Value));");
        sb.AppendLine("    }");
        sb.AppendLine("    public void Dispose() => _strings.Dispose();");
        sb.AppendLine("}");
        sb.AppendLine();
        sb.AppendLine("/// <summary>Authoring entry point: <c>L.T(s =&gt; s.SomeKey)</c> is a deferred");
        sb.AppendLine("/// <see cref=\"global::ZGF.Gui.Prop{T}\"/> that re-resolves on a locale switch.</summary>");
        sb.AppendLine("public static class L");
        sb.AppendLine("{");
        sb.AppendLine("    public static global::ZGF.Gui.Prop<string?> T(global::System.Func<Strings, string> select) =>");
        sb.AppendLine("        global::ZGF.Gui.Prop.Deferred<string?>(ctx => ctx.Localization().Strings.Bind(s => (string?)select(s)));");
        sb.AppendLine("}");
        sb.AppendLine();
        sb.AppendLine("public static class LocalizationWidgetExtensions");
        sb.AppendLine("{");
        sb.AppendLine("    public static ILocalizationService Localization(this global::ZGF.Gui.Context ctx) =>");
        sb.AppendLine("        ctx.Require<ILocalizationService>();");
        sb.AppendLine("}");
    }
