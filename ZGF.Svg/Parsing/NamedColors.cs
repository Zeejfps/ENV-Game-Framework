...
/// <summary>The CSS/SVG named-color table (opaque ARGB).</summary>
internal static class NamedColors
{
    public static bool TryGet(ReadOnlySpan<char> name, out uint argb)
    {
        // Lowercase into a stack buffer; longest CSS color name is 20 chars.
        if (name.Length > 24)
        {
            argb = 0;
            return false;
        ...
        return Table.TryGetValue(lower.ToString(), out argb);
    }

    private static readonly Dictionary<string, uint> Table = new()
    {
